...
export default function SupplierListPage() {
  ...
  const handleDelete = async (id: string) => {
    if (window.confirm('Are you sure you want to delete this supplier?')) {
      try {
        // @ts-ignore
        await window.api.suppliers.delete(id, user?.id);
        toast({
          ...
          <div className="flex items-center gap-3">
            ...
            <div>
              ...
              <div className="text-xs font-bold text-text-secondary uppercase">
                {info.row.original.contactPerson || 'No Contact Person'}
              </div>
            </div>
          </div>
        ),
          ...
          return (
            <div className="flex flex-col items-end">
              <span
                className={cn(
                  'text-sm font-black',
                  balance > 0 ? 'text-red-500' : 'text-text-secondary',
                )}
              >
      ...
      <div className="flex items-center justify-between">
        <div>
          ...
          <p className="text-text-secondary mt-1 font-medium">
            Manage your suppliers, hardware vendors, kcar workshop spareparts, and payments.
          </p>
        </div>
        <Button
      ...
      <Card className="p-8 border-none shadow-soft bg-white rounded-[32px] border border-navy/5">
        <div className="flex flex-col md:flex-row gap-4 mb-8">
          <form onSubmit={handleSearch} className="flex-1 relative group">
            ...
            />
            <Input
              placeholder="Search suppliers by company name, contact, phone..."
        ...
        <div className="mt-8 flex items-center justify-between">
          <p className="text-sm font-bold text-text-secondary">
            Showing <span className="text-navy">{suppliers.length}</span> of{' '}
            <span className="text-navy">{total}</span> suppliers
          </p>
        ...
        <RecordPaymentDialog
          ...
          onSuccess={() => {
            ...
            toast({
              ...
              description: 'Supplier outstanding due has been updated.',
            });
          }}
        />
      )}
